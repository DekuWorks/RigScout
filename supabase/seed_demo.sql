-- RigScout LOCAL DEMO seed only — do NOT apply to production / hosted Supabase.
-- Default `supabase db reset` uses seed.sql (retailers + compatibility rules only).
-- To load this file locally:
--   psql "$DATABASE_URL" -f supabase/seed_demo.sql
--   or temporarily add "./seed_demo.sql" to [db.seed].sql_paths in config.toml
--
